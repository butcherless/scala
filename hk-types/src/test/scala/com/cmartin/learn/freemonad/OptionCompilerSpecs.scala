...
class OptionCompilerSpecs extends AbstractCompilerSpecs {
  ...
  it should "create a CryptoCurrency, Some" in {
    val result: Option[String] =
      create(cryptoCurrency).map(c => c).foldMap(optionCompiler)
  ...
  it should "not create a CryptoCurrency, None" in {
    val result: Option[String] =
      create(existingCryptoCurrency).map(c => c).foldMap(optionCompiler)

    result shouldBe None
  }

  it should "read a CryptoCurrency, Some" in {
    val result: Option[CryptoCurrency] =
      read(currencyName).map(c => c).foldMap(optionCompiler)
  ...
  it should "not read a CryptoCurrency, None" in {
    val result: Option[CryptoCurrency] =
      read(constants.notFoundName).map(c => c).foldMap(optionCompiler)
  ...
  it should "update a CryptoCurrency, Some" in {
    val result: Option[CryptoCurrency] =
      update(cryptoCurrency).map(c => c).foldMap(optionCompiler)
  ...
  it should "not update a CryptoCurrency, None" in {
    val result: Option[CryptoCurrency] =
      update(nonExistingCryptoCurrency).map(c => c).foldMap(optionCompiler)
  ...
  it should "delete a CryptoCurrency, Some" in {
    val result: Option[UUID] =
      delete(cryptoCurrency).map(c => c).foldMap(optionCompiler)
  ...
  it should "not delete a CryptoCurrency, None" in {
    val result: Option[UUID] =
      delete(nonExistingCryptoCurrency).map(c => c).foldMap(optionCompiler)
