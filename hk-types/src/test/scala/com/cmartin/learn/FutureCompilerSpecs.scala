...
class FutureCompilerSpecs extends AbstractCompilerSpecs with ScalaFutures {
  ...
  it should "create a CryptoCurrency" in {
    ...
    whenReady(result) {
      name => name shouldBe currencyName
    }
  }
  ...
  it should "read a CryptoCurrency" in {
    ...
    whenReady(result) {
      cc => cc.name shouldBe currencyName
    }
  }

  it should "not read a CryptoCurrency, Failure" in {
    val result: Future[CryptoCurrency] = read(constants.notFoundName).map(c => c).foldMap(futureCompiler)
  ...
  it should "update a CryptoCurrency" in {
    ...
    whenReady(result) {
      cc => cc shouldBe cryptoCurrency
    }
  }

  it should "not update a CryptoCurrency, Failure" in {
    val result: Future[CryptoCurrency] = update(nonExistingCryptoCurrency).map(c => c).foldMap(futureCompiler)
  ...
  it should "delete a CryptoCurrency" in {
    ...
    whenReady(result) {
      cc => cc shouldBe cryptoCurrency.id
    }
  }
