...
class EitherCompilerSpecs extends AbstractCompilerSpecs {
  ...
  it should "not create a CryptoCurrency, Left" in {
    val result: Either[String, String] = create(existingCryptoCurrency).map(c => c).foldMap(eitherCompiler)
  ...
  it should "not read a CryptoCurrency, Left" in {
    val result: Either[String, CryptoCurrency] = read(constants.notFoundName).map(c => c).foldMap(eitherCompiler)
  ...
  it should "not update a CryptoCurrency, Left" in {
    val result: Either[String, CryptoCurrency] = update(nonExistingCryptoCurrency).map(c => c).foldMap(eitherCompiler)
  ...
  it should "not delete a CryptoCurrency, Left" in {
    val result: Either[String, UUID] = delete(nonExistingCryptoCurrency).map(c => c).foldMap(eitherCompiler)
